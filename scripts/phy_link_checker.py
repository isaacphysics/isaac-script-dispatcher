...
import re
import sys
from constants import PHY_CONTENT_BASE_PATH, OUT_DIR_PATH
...
symbolic_links = {"/pages/fast_track_14_index": "/fast_track_14",
                  "/pages/about_us_index": "/about",
                  "/pages/solving_problems": "/solving_problems",
                  "/pages/privacy_policy": "/privacy",
                  "/pages/apply_uni": "/apply_uni",
                  "/pages/extraordinary_problems_index": "/extraordinary_problems",
                  "/pages/challenge_problems_index": "/challenge_problems",
                  "/pages/prize_draws": "/prize_draws",
                  "/pages/publications": "/publications",
                  "/pages/spc": "/spc",
                  "/pages/why_physics": "/why_physics",
                  "/pages/faq": "/faq",
                  "/pages/bios": "/bios",
                  ...
                  "/pages/mission_teachers": "/mission_teachers",
                  "/pages/mission_students": "/mission_students",
                  "/pages/prize_draws": "/prize_draws",
                  "/pages/chemistry_landing_page": "/chemistry"}

existing_pages = set(["/", "/login", "/register", "/account", "account#emailpreferences",
                      "/progress", "/assignments", "/boards",
                      "/concepts", "/gameboards", "/game_builder",
                      "/assignment_progress", "/set_assignments", "/groups",
                      "/contact", "/teacher_features", "/events",
                      "/book", "/physics_skills_14",
                      "/books/physics_skills_14", "/books/chemistry_16",
                      "/books/phys_book_gcse", "/books/pre_uni_maths",
                      "/exam_uni_help", "/robots.txt"])
existing_pages.update(symbolic_links.values())
...
def parse_link_address(link_address, src_path, link_type):
    ...
    link_address = link_address.replace("https://isaacphysics.org", "").replace("http://isaacphysics.org", "")  # Make any Isaac links local by removing the first part
    if link_address == "":
        return
    if (("/add_gameboard/" in link_address) or ("/board/" in link_address) or ("http" in link_address) or ("www" in link_address) or ("mailto:" in link_address)):
        if ("/add_gameboard/" not in link_address) and ("/board/" not in link_address):  # We don't want to do anything with gameboard links, otherwise
            external_links.add(link_address)  # if it's an external link add it to the list so it can be checked if necessary.
        else:
            gameboard_id = link_address.replace("/board/", "").replace("/add_gameboard/", "")
            gameboard_ids.add(gameboard_id)
        return

    if "/s/" in link_address:  # If it contains a "/s/" then it's a share URL:
        external_links.add("https://isaacphysics.org%s" % link_address)  # Treat it as an external link.
        return
...
def process_thing(json_path, published, page_link, obj):
    ...
    if ("url" in obj) or ("clickUrl" in obj):
        obj_url = obj.get("url") or obj.get("clickUrl")
    ...
    question_types = ["isaacQuestion", "isaacMultiChoiceQuestion", "isaacNumericQuestion", "isaacSymbolicQuestion", "isaacSymbolicChemistryQuestion"]
...
def main(args):
    ...
    all_files = []
    for root, directories, files in os.walk(PHY_CONTENT_BASE_PATH + subdir):
        for fname in files:
    ...
    for fpath in all_files:
        if (not ("biology" in fpath.lower()) or not ignore_biology):
            if fpath.lower().endswith(".json"):
                with open(fpath) as file_handle:
                    try:
